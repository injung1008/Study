...
#L5 - dnn진행 
w5 = tf.get_variable("w5", shape=[2*2*64, 64], initializer=tf.contrib.layers.xavier_initializer()) #[인풋 쉐이프, 아웃풋쉐잎]
b5 = tf.Variable(tf.random_normal([64]), name='bias1') #바이어스 초기값 설정
...
#L6 - dnn진행 
w6 = tf.get_variable("w6", shape=[64, 32], initializer=tf.contrib.layers.xavier_initializer()) #[인풋 쉐이프, 아웃풋쉐잎]
b6 = tf.Variable(tf.random_normal([32]), name='bias2') #바이어스 초기값 설정
...
#L7 - dnn + softmax 진행 
w7 = tf.get_variable("w7", shape=[32, 10]) #[인풋 쉐이프, 아웃풋쉐잎]
b7 = tf.Variable(tf.random_normal([10]), name='bias3') #바이어스 초기값 설정
...
prediction = tf.equal(tf.compat.v1.arg_max(hypothesis,1), tf.arg_max(y,1))
