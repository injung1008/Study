...
] #svc가 갖고 있는 파라미터 C, kernel, gamma 등 이있다  
# 파라미터 "C":[1, 10, 100, 1000 숫자의 값은 본인의 기준으로 넣고 판단을 한다 
#파라미터로 svc의 파라미터 - 수치는 임의로 넣어주는것, 노드의 갯수, 모델등을 선택가능하다 
#파라미터의 종류에 대해서도 선택이 가능 {"c" : [1, 10, 100, 1000], "kernel":["linear"]}, 이거 한줄만 사용해도 돌아간다 
...
#! best_estimator_ 와 best_score_ 는 일반 모델에서는 작동 불가한다


print("model.score :", model.score(x_test, y_test))
    
#! accuracy_score, model.score 같은 값을 낸다 

y_pred = model.predict(x_test)
print("accuracy_score : ", accuracy_score(y_test, y_pred))
#  test 데이터이기떄문에 best_score랑accuracy_score 다르다 
